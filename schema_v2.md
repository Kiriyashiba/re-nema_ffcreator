...
```jsonc
"outro": { "enabled": true, "cta": "台本ください→プロフィールへ", "duration": 3.0 }
```

- 末尾に専用シーンを1つ追加。`composeCanvasPng` をそのまま使うので新しい仕組みが要らない
- **クレジットは自動生成**。その動画で実際に使われた全話者の `credit` を収集して連結する
- 常時表示の小さなクレジットは**各シーンのキャンバスに焼き込む**。
  動画全体に後からオーバーレイすると `addBgm` の `-c:v copy` が使えなくなり全編再エンコードになる
- `duration` は必須（読み上げが無いシーンのため）
