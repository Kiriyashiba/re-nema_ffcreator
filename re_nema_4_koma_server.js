"use strict";
/*
 Re:NEMA 4コマAPI（LITE 安定版）
 - 4シーン（空は自動スキップ）
 - 画像/動画コンテンツ、上下テキスト（同時表示）
 - 読み上げON/OFF、シーンSFX（sounds/sfx 参照・プレビュー）、全体BGM（sounds/bgm 参照・プレビュー）
 - 効果は fade 固定（in/out 各 0.2s）← 要望の「半分」
 - 最低時間は 5 秒固定（UI非表示）※音声は“切らない”。映像尺を音声に合わせる
 - 背景色は「全体のみ」
 - 元動画の音声は「OFF の場合にチェック」（muteSrcAudio）
 - VOICEVOX スピーカー一覧をフォームで選択（/api/voicevox/speakers）
 - セキュリティ: APIキー, contest token, CORS, レート制限, SSRF(DNS+Private IP遮断), MIME/サイズ制限
 - 依存: ffmpeg, ffprobe, ImageMagick(convert, composite, identify)
...
const SFX_DIR = "/root/re-nema_ffcreator/sounds/sfx"; // 小文字に統一
const BGM_DIR = "/root/re-nema_ffcreator/sounds/bgm"; // 小文字に統一
...
// 追加: 日本語長文でも確実に折り返すための簡易ラッパー
function softWrapText(text, maxCharsPerLine){
  const result = [];
  const lines = String(text||"").replace(/\r/g, "").split("\n");
  const forbidHead = /[、。,.，．）〕〉》」』】!?！？・:：；、。]/; // 行頭に来て欲しくない記号

  for (const raw of lines){
    const chars = [...raw]; // サロゲート対応
    let buf = "";
    for (let i = 0; i < chars.length; i++){
      buf += chars[i];
      if (buf.length >= maxCharsPerLine){
        // 次行頭が句読点等にならないよう、可能なら今行に1?2文字取り込む
    ...
    if (chars.length === 0) result.push(""); // 空行維持
  }
  return result.join("\n");
}
...
// ====== テキストPNG（caption: 自動改行 + 簡易フィット） ======
// 置き換え: テキストPNG（自動折り返し + フィット）
async function textCaptionToPng(text, width, boxH, align, outPath, colorHex){
  ...
  // 初期ポイントサイズ（必要に応じて自動で縮める）
  let pt = 56;

  for (let tries = 0; tries < 6; tries++){
    // 1) そのポイントサイズで「1行に収まる概算文字数」を見積もって改行を挿入
    //    係数0.58は NotoSansJP の字幅の経験値（必要なら微調整可）
    const maxChars = Math.max(4, Math.floor(tgtW / (pt * 0.58)));
    const wrapped = softWrapText(text, maxChars);

    // 2) 一時描画（caption: で自動折り返し最終調整）
...
// SFXを音声に重ねる（声は“切らない”。出力は声の長さちょうど）
async function mixSfxOverVoice(outPath, voiceWav, sfxObj){
  ...
  // 声の長さに合わせる：SFXは切る、声は切らない
  const filter =
    `[0:a]asetpts=PTS-STARTPTS[v];`+
    `[1:a]volume=${vol},atrim=end=${D},asetpts=PTS-STARTPTS[s];`+
    `[v][s]amix=inputs=2:duration=first[a]`;
  const cmd = `ffmpeg -y -i "${voiceWav}" -i "${sfxPath}" -filter_complex "${filter}" -map "[a]" -ar 48000 -ac 2 -f wav "${outPath}"`;
  await exec(cmd);
  return outPath;
}
...
async function renderImageSilent(outMp4, W,H,FPS, scene, duration, bgDefault){
  ...
  await exec(`ffmpeg -y -loop 1 -i "${canvas}" -t ${D} -r ${FPS} -vf "format=yuv420p,setpts=PTS-STARTPTS" -c:v libx264 -pix_fmt yuv420p -profile:v baseline -level 3.1 "${outMp4}"`);
  return outMp4;
}

// ====== 動画シーン（文字は全尺常時表示 / フェードは mux 段） ======
...
async function buildSceneAudio(scene, speakerDefault, FPS){
  ...
  // 2) 連結 → 5秒に満たない場合は「差分だけ」無音を末尾に連結（★バグ修正点）
  ...
  if (wavs.length === 0) {
    ...
    if (d0 < minD - 0.001) {
      ...
      // 元(voice) + 無音(pad秒) の 2本だけを concat（前後5秒丸ごと付加しない）
      await exec(
        `ffmpeg -y -i "${voice}" -f lavfi -t ${pad.toFixed(3)} -i anullsrc=r=48000:cl=stereo ` +
...
async function muxVideoAudio(outMp4, silentMp4, audioPath, duration, effect, srcAudio){
  const fin = (effect && effect.in) ? Number(effect.in) : 0.2;   // ← 半分
  const fout= (effect && effect.out)? Number(effect.out) : 0.2;   // ← 半分
  const D = Math.max(1, Math.floor(duration||5));

  if (srcAudio){
    // 元動画音声は D 秒で切る（声は切らない）
    const filter = `[0:v]setpts=PTS-STARTPTS,fade=t=in:st=0:d=${fin},fade=t=out:st=${Math.max(0,D-fout)}:d=${fout},format=yuv420p[v];`+
...
async function concatVideos(files, finalOut){
  ...
  await exec(`ffmpeg -y -safe 0 -f concat -i "${list}" -fflags +genpts -af aresample=async=1:first_pts=0 -c:v libx264 -preset veryfast -crf 22 -c:a aac -ar 48000 "${finalOut}"`);
  return finalOut;
}

// --- シンプル&安定：BGM合成（まずamixを試し、失敗なら“BGMだけ載せる”）
async function addBgm(finalIn, bgm){
  ...
  const fallback =
    `ffmpeg -y -i "${finalIn}" ${loopOpt} -i "${bgPath}" ` +
    `-filter_complex "[1:a]volume=${vol},aresample=48000[a]" ` +
    `-map 0:v -map "[a]" -c:v copy -c:a aac -shortest "${out}"`;
...
async function renderFromConfig(cfg){
  ...
  for (let i=0;i<scenes.length && i<4;i++){
    const sc = scenes[i];
    const part = await buildSceneAudio(sc, spk, FPS);
    const D = Math.max(1, Math.floor(part.duration||5)); // ← 音声に合わせた尺（音声は切らない）
    ...
    const srcA = (sc.useSrcAudio && vLocalRef)
      ? (await (async()=>{ const out = path.join(TMP_DIR, `srca_${stamp()}.wav`); await exec(`ffmpeg -y -i "${vLocalRef}" -vn -t ${D} -ar 48000 -ac 2 -f wav "${out}"`); return out; })())
      : "";
...
app.get("/health", async (req,res)=>{
  ...
  res.json({ ok:true, port:PORT, outDir:OUT_DIR, tmpDir:TMP_DIR, ffmpeg:ff, imagemagick:im, voicevox: VOICEVOX_BASE, font: FONT_PATH, sfxDir:SFX_DIR, bgmDir:BGM_DIR });
});

// VoiceVox スピーカー一覧（フォームから参照）
...
.cta{display:inline-block;padding:12px 18px;border-radius:999px;background:linear-gradient(135deg,var(--accent),var(--accent2));color:#0a0f1a;font-weight:700;text-decoration:none;box-shadow:var(--shadow)}
...
label.cb{display:inline-flex;align-items:center;gap:.5rem;font-weight:600}
  ...
  <div class="container">
    <div class="card pad">
      ...
      <p class="muted">最低時間は各シーン5秒固定・効果はフェード固定（0.2s）。背景色は全体設定で一括。</p>
      <form method="POST" action="/contest/4koma-lite${tokenQs}" enctype="multipart/form-data">
        ...
        <section class="row">
          ...
          <div class="grid" style="margin-top:.6rem">
            <div>
              <label>VOICEVOX speaker</label>
              <select id="speakerSel" name="speakerId"><option value="2">(読み込み中)…</option></select>
            </div>
            <div>
  ...
  <script>
    ...
    const pick=document.getElementById('bgColorPick'); const txt=document.getElementById('bgColorText');
    pick.addEventListener('input',()=>{ txt.value=pick.value; });
    txt.addEventListener('input',()=>{ if(/^#?[0-9a-fA-F]{6}$/.test(txt.value)){ txt.value=txt.value.startsWith('#')?txt.value:('#'+txt.value); pick.value = txt.value; }});

    function bindPreview(selId,audId,base){
      const sel=document.getElementById(selId); const aud=document.getElementById(audId);
      if(!sel||!aud) return;
      sel.addEventListener('change',()=>{ const v=sel.value; aud.src = v? (base + encodeURIComponent(v)) : ''; aud.pause(); if(v){ aud.load(); } });
    }
    bindPreview('bgmSel','bgmAud','/media/bgm/');
    ${[1,2,3,4].map(i=>`bindPreview('sfxSel_${i}','sfxAud_${i}','/media/sfx/');`).join('')}

    // VOICEVOX speakers を取得して <select> に反映
    (async ()=>{
      try{
        const r = await fetch('/api/voicevox/speakers'+(token?('?token='+token):''));
        const j = await r.json(); const sel=document.getElementById('speakerSel'); if(!sel) return;
        sel.innerHTML='';
        if(j.ok && Array.isArray(j.speakers) && j.speakers.length){
          j.speakers.forEach(sp=>{
            const opt=document.createElement('option');
            opt.value=String(sp.styleId);
            opt.textContent=sp.speakerName+' / '+sp.styleName+' ('+sp.styleId+')';
            sel.appendChild(opt);
          });
        } else {
          const opt=document.createElement('option'); opt.value='2'; opt.textContent='デフォルト(2)'; sel.appendChild(opt);
        }
      }catch(e){
        const sel=document.getElementById('speakerSel'); if(sel){ sel.innerHTML='<option value="2">デフォルト(2)</option>'; }
      }
    })();
  </script>`);
});
...
app.post("/contest/4koma-lite", contestGuard, upload.any(), async (req,res)=>{
  try{
    ...
    for (let i=1;i<=4;i++){
      ...
      const muteSrc = !!B['muteSrcAudio_'+i]; // チェック＝オフ
...
app.post("/api/render", auth, bodyParser.json({limit:"10mb"}), bodyParser.urlencoded({ extended:true, limit:"10mb" }), async (req,res)=>{
  try{
    ...
    const out = await renderFromConfig(cfg);
    const file = path.basename(out);
    res.json({ ok:true, file, url:"/output/"+file });
  }catch(e){ res.status(400).json({ ok:false, error: errStr(e) }); }
});
...
async function workerOnce(id){
  ...
  j.status="working";
  try{
    const out = await renderFromConfig(j.payload);
    const file = path.basename(out);
    j.status="done"; j.result={ file, url:"/output/"+file };
  }catch(e){ j.status="error"; j.error=errStr(e); }
}
async function pump(){
...
appServer.listen(PORT, ()=>{ console.log("4koma server listening on :"+PORT); });
